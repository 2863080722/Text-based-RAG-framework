...
# ----------------------------------------------------------------------------------------------------------------------
# 遍历每一个问题，分别得到BM25与BGE检索下所得到的问题与页面内容的相似度（即评分）
for query_idx, feat in enumerate(question_embeddings):
    ...
    max_score_page_idxs = score.argsort()[-4:]
    ...
    # 得到重排序的结果，通过scores.cpu().numpy().argmax()找到最大项在TOP3中选取匹配度最高的前两个文本
    top_two_idxs = scores.cpu().numpy().argsort()[-2:][::-1]
    top_two_page_idxs = [max_score_page_idxs[i] for i in top_two_idxs]  # 将索引转换为原始的页面索引
    questions[query_idx]['reference'] = 'page_' + str(top_two_page_idxs[0] + 1)
    ...
    问题：{2} 
    '''.format(
  pdf_content[top_two_page_idxs[0]]['content'],  # 第一页内容
        pdf_content[top_two_page_idxs[1]]['content'],  # 第二页内容
